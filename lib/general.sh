...
# build aarch64
	...
	if [[ -z $HOSTRELEASE || "buster bullseye focal impish hirsute debbie tricia ulyana ulyssa uma jammy" != *"$HOSTRELEASE"* ]]; then
...
# build aarch64
  if [[ $(dpkg --print-architecture) == amd64 ]]; then
	...
	if [[ -z $HOSTRELEASE || $HOSTRELEASE =~ ^(focal|debbie|buster|bullseye|impish|hirsute|ulyana|ulyssa|uma|jammy)$ ]]; then
